...
public protocol PermissionDeniedCallBack {
  func onCancelClick()
}
...
public class YPLibraryVC: UIViewController, YPPermissionCheckable {
  ...
  var callBackDelegate: PermissionDeniedCallBack? = nil
...
//        multipleSelectionEnabled = !multipleSelectionEnabled
  ...
  func checkPermissionToAccessPhotoLibrary(block: @escaping (Bool) -> Void) {
    ...
    case .restricted, .denied:
      ...
      let alert = popup.popup(cancelBlock: { [weak self] in
        self?.callBackDelegate?.onCancelClick()
        block(false)
      })
